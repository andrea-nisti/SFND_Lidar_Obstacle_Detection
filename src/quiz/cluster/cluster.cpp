...
void euclideanClusterHelper(const std::vector<std::vector<float>> &points, std::vector<int> &cluster, KdTree *tree, std::vector<bool> &visited, const size_t point_id, float distanceTol)
{
	...
	std::cout << "neighbors: ";
	std::for_each(neighbors.cbegin(), neighbors.cend(), [](const auto& n){std::cout << n << " ";});
	std::cout << std::endl;
	for (const auto &neighbor_id : neighbors)
	{
		if(not visited.at(neighbor_id)) euclideanClusterHelper(points, cluster, tree, visited, neighbor_id, distanceTol);
	}
...
std::vector<std::vector<int>> euclideanCluster(const std::vector<std::vector<float>> &points, KdTree *tree, float distanceTol)
{
	...
	for (const auto &point : points)
	{
		if (not visited.at(id))
		{
			...
			std::cout << "Curr point: " << id << " "<< point.at(0) << "|" << point.at(1) << std::endl;
